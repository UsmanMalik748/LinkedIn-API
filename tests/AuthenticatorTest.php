...
class AuthenticatorTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        return m::mock('Usman\LinkedInApi\Http\RequestManager');
    }
    ...
    public function testGetLoginUrl()
    {
        ...
        $storage = $this->getMock('Usman\LinkedInApi\Storage\DataStorageInterface');
        $storage->method('get')->with('state')->willReturn($state);

        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['establishCSRFTokenState', 'getStorage'], [$this->getRequestManagerMock(), self::APP_ID, self::APP_SECRET]);
        ...
        $generator = m::mock('Usman\LinkedInApi\Http\LinkedInUrlGeneratorInterface')
    ...
    {
        $generator = m::mock('Usman\LinkedInApi\Http\UrlGenerator');
        $code = 'newCode';
        $storage = m::mock('Usman\LinkedInApi\Storage\DataStorageInterface')
            ->shouldReceive('set')->once()->with('code', $code)
            ->shouldReceive('set')->once()->with('access_token', 'at')
            ->getMock();

        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['getCode', 'getStorage', 'getAccessTokenFromCode'], [], '', false);
    ...
    /**
     * @expectedException \Usman\LinkedInApi\Exception\LinkedInException
     */
    ...
    public function testFetchNewAccessTokenNoCode()
    {
        ...
        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['getCode', 'getStorage'], [], '', false);
        $auth->expects($this->any())->method('getStorage')->will($this->returnValue($storage));
    ...
    public function testGetAccessTokenFromCodeEmptyString()
    {
        ...
        $method = new \ReflectionMethod('Usman\LinkedInApi\Authenticator', 'getAccessTokenFromCode');
        $method->setAccessible(true);
        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', [], [], '', false);
    ...
    protected function prepareGetAccessTokenFromCode($code, $responseData)
    {
        ...
        $requestManager = m::mock('Usman\LinkedInApi\Http\RequestManager')
        ...
        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['getStorage'], [$requestManager, self::APP_ID, self::APP_SECRET]);
        $auth->expects($this->any())->method('getStorage')->will($this->returnValue($storage));
    ...
    {
        $method = new \ReflectionMethod('Usman\LinkedInApi\Authenticator', 'establishCSRFTokenState');
        $method->setAccessible(true);
        ...
        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['getStorage'], [], '', false);
        $auth->expects($this->any())->method('getStorage')->will($this->returnValue($storage));
    ...
    public function testGetCodeEmpty()
    {
        ...
        $method = new \ReflectionMethod('Usman\LinkedInApi\Authenticator', 'getCode');
        $method->setAccessible(true);
        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', [], [], '', false);
    ...
    {
        $method = new \ReflectionMethod('Usman\LinkedInApi\Authenticator', 'getStorage');
        $method->setAccessible(true);
        $requestManager = $this->getRequestManagerMock();
        $auth = new Authenticator($requestManager, self::APP_ID, self::APP_SECRET);

        // test default
        $this->assertInstanceOf('Usman\LinkedInApi\Storage\SessionStorage', $method->invoke($auth));

        $object = m::mock('Usman\LinkedInApi\Storage\DataStorageInterface');
