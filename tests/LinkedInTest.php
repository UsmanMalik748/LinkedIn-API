...
class LinkedInTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testApi()
    {
        ...
        $generator = $this->getMock('Usman\LinkedInApi\Http\UrlGenerator', ['getUrl']);
        ...
        $requestManager = $this->getMock('Usman\LinkedInApi\Http\RequestManager', ['sendRequest']);
        ...
        $linkedIn = $this->getMock('Usman\LinkedInApi\LinkedIn', ['getAccessToken', 'getUrlGenerator', 'getRequestManager'], [self::APP_ID, self::APP_SECRET]);
    ...
    {
        $linkedIn = $this->getMock('Usman\LinkedInApi\LinkedIn', ['getAccessToken'], [self::APP_ID, self::APP_SECRET]);
        $linkedIn->expects($this->once())->method('getAccessToken')->willReturn(null);
        $this->assertFalse($linkedIn->isAuthenticated());

        $linkedIn = $this->getMock('Usman\LinkedInApi\LinkedIn', ['api', 'getAccessToken'], [self::APP_ID, self::APP_SECRET]);
        $linkedIn->expects($this->once())->method('getAccessToken')->willReturn('token');
    ...
    public function testAccessTokenAccessors()
    {
        ...
        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['fetchNewAccessToken'], [], '', false);
        $auth->expects($this->once())->method('fetchNewAccessToken')->will($this->returnValue($token));

        $linkedIn = $this->getMock('Usman\LinkedInApi\LinkedIn', ['getAuthenticator'], [], '', false);
    ...
    {
        $get = new \ReflectionMethod('Usman\LinkedInApi\LinkedIn', 'getUrlGenerator');
        $get->setAccessible(true);
        $linkedIn = new LinkedIn(self::APP_ID, self::APP_SECRET);

        // test default
        $this->assertInstanceOf('Usman\LinkedInApi\Http\UrlGenerator', $get->invoke($linkedIn));

        $object = $this->getMock('Usman\LinkedInApi\Http\UrlGenerator');
    ...
    {
        $get = new \ReflectionMethod('Usman\LinkedInApi\LinkedIn', 'getFormat');
        $get->setAccessible(true);
        $linkedIn = new LinkedIn(self::APP_ID, self::APP_SECRET);
    ...
    public function testLoginUrl()
    {
        ...
        $generator = $this->getMock('Usman\LinkedInApi\Http\UrlGenerator', ['getCurrentUrl']);
        $generator->expects($this->once())->method('getCurrentUrl')->willReturn($currentUrl);

        $auth = $this->getMock('Usman\LinkedInApi\Authenticator', ['getLoginUrl'], [], '', false);
        $auth->expects($this->once())->method('getLoginUrl')
            ->with($generator, ['redirect_uri' => $currentUrl])
            ->will($this->returnValue($loginUrl));

        $linkedIn = $this->getMock('Usman\LinkedInApi\LinkedIn', ['getAuthenticator', 'getUrlGenerator'], [], '', false);
        $linkedIn->expects($this->once())->method('getAuthenticator')->willReturn($auth);
        $linkedIn->expects($this->once())->method('getUrlGenerator')->willReturn($generator);
    ...
    public function testLoginUrlWithParameter()
    {
        ...
        $generator = $this->getMock('Usman\LinkedInApi\Http\UrlGenerator');
